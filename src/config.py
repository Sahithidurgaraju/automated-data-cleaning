from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parent.parent
DATA_DIR = PROJECT_ROOT / "data"
JSON_DIR = PROJECT_ROOT / "json_output"
CLEANED_DATA_DIR = PROJECT_ROOT/"cleaned_data_output"
PLOTS_DIR = PROJECT_ROOT/"plots"
VALIDATION_DIR = PROJECT_ROOT/"validation_reports"
DATABASE_DIR = PROJECT_ROOT/"sql_credentials"
CLEANED_TRANFORM_DATA_DIR = PROJECT_ROOT/"transformed_cleaned_data_output"
DASHBOARD_DIR = PROJECT_ROOT/"dashboard_reports"
CA_PATH = PROJECT_ROOT / "certs" / "isrgrootx1.pem"